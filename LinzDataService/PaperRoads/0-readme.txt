...
Tilename-extras.txt are lists of Linzid's that are actually valid, but don't appear on the Linz map. This is probably only for edge cases - there is only currently one entry. 
...
Tilename-LINZWrongside.txt are lists of Addresses that appear on the wrong side of the road in LINZ data. These should be sent to LINZ for evaluation, and hopefully correction. The idea is "We've told LINZ. Now we can ignore it"
