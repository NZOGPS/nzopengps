...
if /i %1 equ Canterbury	%nzogps_psqlc% -c "drop table if exists %1_nums; Create table %1_Nums as select * from \"nz-street-address-elector\" where st_y(the_geom)<-42.731949 and st_y(the_geom)>-45.055931 and st_x(the_geom) > 0;"
if /i %1 equ Southland	%nzogps_psqlc% -c "drop table if exists %1_nums; Create table %1_Nums as select * from \"nz-street-address-elector\" where st_y(the_geom)<-45.055931;"
