...
copy(select st_x(st_startpoint(the_geom)),st_y(st_startpoint(the_geom)),label,st_length(nztm_line)/least_nums from :linestable where st_length(nztm_line)/least_nums > :distance order by 4 desc) to :outfile with CSV;