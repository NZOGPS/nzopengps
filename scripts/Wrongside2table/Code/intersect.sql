select current_time;
\echo clear values
update :numstable set (asnum_roadid,asnum_distance,asnum_position,asnum_dist_err,isect_side)=(null,null,null,null,null); --clear values in case road has been edited
\echo left road_id, side, segment
update :numstable set asnum_roadid=:linestable.gid,asnum_side=-1,asnum_segment=nnum  from :linestable where  :numstable.road_id = :linestable.linzid and gt_within(address_number,lstart,lend,ltype) ;
\echo right road_id, side, segment
update :numstable set asnum_roadid=:linestable.gid,asnum_side=1,asnum_segment=nnum  from :linestable where  :numstable.road_id = :linestable.linzid and gt_within(address_number,rstart,rend,rtype) ;
\echo left asnum_distance
update :numstable set asnum_distance=gt_distance(address_number,asnum_side,lstart,lend) from :linestable where :linestable.gid=asnum_roadid and asnum_side=-1;
\echo right asnum_distance
update :numstable set asnum_distance=gt_distance(address_number,asnum_side,rstart,rend) from :linestable where :linestable.gid=asnum_roadid and asnum_side=1;
\echo asnum_position
update :numstable set asnum_position=ST_LineInterpolatePoint(nztm_line, asnum_distance) from :wrongstable where :wrongstable.gid=asnum_roadid;
\echo asnum_dist_err
update :numstable set asnum_dist_err=ST_Distance(nztm, asnum_position);
\echo left intersects right
update :numstable set isect_side=1, isect_roadid=:wrongstable.gid from :wrongstable where road_id=linzid and ST_Intersects(nztm,rightpoly);
\echo right intersects left 
update :numstable set isect_side=-1, isect_roadid=:wrongstable.gid from :wrongstable where road_id=linzid and ST_Intersects(nztm,leftpoly);
copy(select shape_x,shape_y,full_address_ascii,'Wrong Side: ' || address_id from :numstable where asnum_roadid = isect_roadid and asnum_side*isect_side=-1 order by address_id) to :outfile with CSV;
select current_time;
