...
CREATE INDEX idx_road_id ON "nz-street-address-electoral" USING btree (road_id);
CREATE INDEX idx_road_id_is_odd ON "nz-street-address-electoral" USING btree (road_id,is_odd);
=====
Menu -> Query -> Execute
...
def pre_processing()
  ...
  print "Output : #{@output_file_path}\n"
	...
	@i = 0
	@stats = Hash.new(0)
	@xlt_worked = Hash.new(0)
	@geom_worked = Hash.new(0)
  ...
  @do_ciyidx = have_cityidx
#  puts "docity = #{@do_ciyidx}..."
  if @do_ciyidx then
    @address_table = make_city_address_table
  end
end
...
def process_polish_buffer(buffer)
  ...
  @pbar.increment
	...
	if ($1 =~ /-Regions/) then
		...
		id_set = $1
	elsif line =~ /^\;linz_region\=(.*)/ then
		region = $1
	elsif line =~ /^\;linz_locality\=(.*)/ then
		locality = $1
...
#    print "run_numberer #{id_set}\n"
		...
		numberBuffer = []
		if @do_ciyidx then
			cityIdx = findCityIdx(region,locality,data0)
			if cityIdx != 0 then
				numberBuffer << "CityIdx=#{cityIdx}"
				@stats["count_of_sections_city_indexed"]+=1
			end
		end
	...
	#################
	if is_header and @do_ciyidx then
		insert_address_table(buffer)
	end
...
###############################
def insert_address_table(buffer)
	rgnend = buffer.index('[END-Regions]')
	if rgnend then
		buffer.insert(rgnend+1,@address_table)
	else
...
def nearest_address_sql_fullside(road_id,point1x,point1y,point2x,point2y,side1x,side1y,side2x,side2y,originalside)
  sql_query = <<-EOS
SELECT #{STREET_ADDRESS_NUM_COL}, road_id, ST_Distance(ST_SetSRID(ST_MakePoint(#{point1x},#{point1y}), #{WORKING_SRID}),#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE
...
dbg = false
@reporting_file.puts ('cd2wkt d0: '+data0) if dbg

if data0[0] == '(' then # added GCT 20150913 when using for city indexing as the (...) had already been stripped in a previous call
...
@reporting_file.puts('cd2wkt pgc1: '+postgis_compatible) if dbg
...
@reporting_file.puts('cd2wkt wkt: '+wkt) if dbg
...
#############
def get_nearest_address_inside_polygon(road_id,polygon_geom,first_point)
#ST_Contains(geometry A, geometry B)
#    Returns 1 (TRUE) if Geometry A "spatially contains" Geometry B.
  sql_query = <<-EOS
SELECT #{STREET_ADDRESS_NUM_COL}, road_id, ST_Distance(#{first_point},#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE
...
def get_odd_or_not_count(road_id,polygon_geom,is_odd)
  sql_query = <<-EOS
SELECT count(*)
...
##############
def get_odd_even_counts(road_id,polygon_geom)

odd = get_first_result(get_odd_or_not_count(road_id,polygon_geom,true)).to_i
even = get_first_result(get_odd_or_not_count(road_id,polygon_geom,false)).to_i
return [odd,even]
...
##############
def count_houses_on_street(road_id)

  sql_query = "SELECT COUNT(*) FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id};"
  return get_first_result(sql_query).to_i

end
###
def get_low_high_on_street(road_id)

sql_query = "SELECT min(#{STREET_ADDRESS_NUM_COL}) FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = TRUE;"
min_odd = get_first_result(sql_query).to_i

sql_query = "SELECT min(#{STREET_ADDRESS_NUM_COL}) FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = FALSE;"
min_even = get_first_result(sql_query).to_i

sql_query = "SELECT max(#{STREET_ADDRESS_NUM_COL}) FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = TRUE;"
max_odd = get_first_result(sql_query).to_i

sql_query = "SELECT max(#{STREET_ADDRESS_NUM_COL}) FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = FALSE;"
max_even = get_first_result(sql_query).to_i

return [min_odd,min_even,max_odd,max_even]
end
###
def get_low_high_odds_evens_plus_distance_on_street(road_id,first_point)
  
sql_query = "SELECT #{STREET_ADDRESS_NUM_COL}, ST_Distance(ST_SetSRID(#{first_point}, #{WORKING_SRID}),#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = TRUE ORDER BY #{STREET_ADDRESS_NUM_COL} ASC LIMIT 1;"
min_odd, min_odd_distance = get_number_and_distance(sql_query)

sql_query = "SELECT #{STREET_ADDRESS_NUM_COL}, ST_Distance(ST_SetSRID(#{first_point}, #{WORKING_SRID}),#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = FALSE ORDER BY #{STREET_ADDRESS_NUM_COL} ASC LIMIT 1;"
min_even, min_even_distance = get_number_and_distance(sql_query)

sql_query = "SELECT #{STREET_ADDRESS_NUM_COL}, ST_Distance(ST_SetSRID(#{first_point}, #{WORKING_SRID}),#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = TRUE ORDER BY #{STREET_ADDRESS_NUM_COL} DESC LIMIT 1;"
max_odd, max_odd_distance = get_number_and_distance(sql_query)

sql_query = "SELECT #{STREET_ADDRESS_NUM_COL}, ST_Distance(ST_SetSRID(#{first_point}, #{WORKING_SRID}),#{STREET_ADDRESS_TABLE_INDEX_NAME}) AS distance FROM #{STREET_ADDRESS_TABLE} WHERE road_id = #{road_id} AND is_odd = FALSE ORDER BY #{STREET_ADDRESS_NUM_COL} DESC LIMIT 1;"
...
def nearest_address_working_single_section(road_id,data0)
  ...
  count_of_houses_on_street = count_houses_on_street(road_id)
  ...
  left_odd_count, left_even_count = get_odd_even_counts(road_id,left_polygon_geom)
  right_odd_count, right_even_count = get_odd_even_counts(road_id,right_polygon_geom)

  #min_odd,min_even,max_odd,max_even = get_low_high_on_street(road_id)
    
  if (left_odd_count > 0 && right_odd_count > 0) && (left_even_count > 0 && right_even_count > 0) then
    # is probably mixed

    right_first, right_first_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,right_polygon_geom,first_point))
    right_last, right_last_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,right_polygon_geom,last_point))

    left_first, left_first_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,left_polygon_geom,first_point))
    left_last, left_last_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,left_polygon_geom,last_point))
  ...
  min_odd,min_odd_distance,min_even,min_even_distance,max_odd,max_odd_distance,max_even,max_even_distance = get_low_high_odds_evens_plus_distance_on_street(road_id,first_point)
  ...
  #check_parity_of_neighbours(road_id,first_point) #stupid method, useless

end

##########

def nearest_address_working(road_id,data0,numberIndex = 1)
# numberIndex
  ...
  #right_first = get_nearest_address_inside_polygon(road_id,right_polygon_geom,first_point)
  right_first, right_first_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,right_polygon_geom,first_point))
  right_last, right_last_parity = return_neat_result(get_nearest_address_inside_polygon(road_id,right_polygon_geom,last_point))
  ...
  nearest_number = get_nearest_address_inside_buffer(road_id,wkt,first_point)
...
def get_nearest_address_inside_buffer(road_id,polygon_geom,first_point)
...
def check_parity_of_neighbours(road_id,first_point)
...
#########################
def table_exists (te_tablename)
	begin
		sql_query = "SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename  = '#{te_tablename}');"
		res = @conn.exec(sql_query)
		have_table = res.entries[0]["exists"]=='t'
		return have_table
	end
end
...
def have_cityidx
	begin
		have_burbs_table = table_exists(SUBURBS_TABLE)
		have_cities_table = table_exists(@tile.downcase+"_cities")
...
#########################
def get_cityidx_from_cities(locl,regn)
	begin
		...
		res = @conn.exec(sql_query)
		if res.num_tuples > 1 then
			@reporting_file.puts("#{res.num_tuples} tuples from #{sql_query}")
			res.each do |row|
				@reporting_file.puts("row: "+row.to_s)
		end
	end
...
#########################
def get_cityidx_from_cities_and_xlt(locl,regn)
	begin
		sql_query = "SELECT cityid from "+@tile.downcase+"_cities join #{SUBURBS_TABLE} on nzslid=id join "+@tile.downcase+"_xlt_rsa_sal on name_ascii=nzsal_na where nzrsa_na='#{locl}' and territorial_authority_ascii like '%#{regn}%' and secondary = 0;"
		res = @conn.exec(sql_query)
		return (res)
	end
end
...
#########################
def get_cityidx_from_geometry(data0)
	begin
		wkt = convert_data0_to_wkt(data0)
		# first try contains
		sql_query = "SELECT cityid from "+@tile.downcase+"_cities join #{SUBURBS_TABLE} on nzslid=id where ST_Contains(#{SUBURBS_TABLE_GEOM},#{wkt})"
		@reporting_file.puts (sql_query)
		res = @conn.exec(sql_query)
		if res.num_tuples == 1 then 
			return (res)
		else # then try intersects
			sql_query = "SELECT cityid from "+@tile.downcase+"_cities join #{SUBURBS_TABLE} on nzslid=id where ST_Intersects(#{SUBURBS_TABLE_GEOM},#{wkt})"
			@reporting_file.puts (sql_query)
			res = @conn.exec(sql_query)
			if res.num_tuples == 1 then 
				return (res)
			else # multiple intersects? Calculate overlap length
				if res.num_tuples > 1 then 
					sql_query = "SELECT cityid, st_length(st_intersection(#{SUBURBS_TABLE_GEOM},#{wkt})) from "+@tile.downcase+"_cities join #{SUBURBS_TABLE} on nzslid=id where ST_Intersects(#{SUBURBS_TABLE_GEOM},#{wkt}) order by st_length DESC"
					@reporting_file.puts (sql_query)
					res = @conn.exec(sql_query)
					return (res)
				else
...
#########################
def make_city_address_table
table = "\n[Cities]\n"
	begin
		sql_query = "SELECT label, city, cityid, rgnidx FROM "+@tile.downcase+"_cities ORDER BY cityid;"
		res  = @conn.exec(sql_query)
		res.each do |row|
			table << "City#{row["cityid"]}=#{row["label"]}"
			if !row["city"].empty? then
				table << ", #{row["city"]}\n"
			else
				table << "\n"
			end
			table << "RegionIdx#{row["cityid"]}=#{row["rgnidx"]}\n"
		end
		table << "[END-Cities]\n"
	end
	return table
...
#########################
def findCityIdx(regn,locl,data0)
	cityid = 0
	#escape single quotes for sql
	locl.gsub!("'","''")
	regn.gsub!("'","''")
	res = get_cityidx_from_cities(locl,regn)
	if res.num_tuples == 1 then
		cityid = res.entries[0]["cityid"]
#		@reporting_file.puts("#{locl} is #{cityid}")
	else
		if res.num_tuples == 0 and table_exists(@tile.downcase+"_xlt_rsa_sal") then
			res = get_cityidx_from_cities_and_xlt(locl,regn)
			if res.num_tuples == 1 then
				cityid = res.entries[0]["cityid"]
				@xlt_worked[locl]+=1
				#@reporting_file.puts("Second try with #{sql_query} worked")
			else
				res = get_cityidx_from_geometry(data0)
				res.each do |row|
					@reporting_file.puts ("gcfg row: "+row.to_s)
				end
				if res.num_tuples == 1 then
					cityid = res.entries[0]["cityid"]
					@geom_worked[locl]+=1
				end
				if res.num_tuples > 1 and res.entries[0]["st_length"] then # if there's a length, there's a couple of intersections
					cityid = res.entries[0]["cityid"] # 0 is the longest overlap
					@geom_worked[locl]+=1
				end
			end
		end
