CREATE INDEX idx_road_id_p ON nz-addresses-road-sections-pilot USING btree (road_id);

-- below is psql command to emit only tuples on selects for timing
\t 
SELECT 'ALTER TABLES',NOW();
ALTER TABLE nz-addresses-pilot ADD COLUMN is_odd boolean;
ALTER TABLE nz-addresses-pilot ADD COLUMN road_section_id integer;
ALTER TABLE nz-addresses-pilot ADD COLUMN rna_id integer;
ALTER TABLE nz-addresses-pilot ADD COLUMN linz_numb_id integer;

SELECT 'IS ODD',NOW(); -- 4 min on 20230304
UPDATE nz-addresses-pilot    SET is_odd = MOD(address_number,2) = 1;

SELECT 'ROAD SECTION ID',NOW(); -- 47 min on 20230304
UPDATE nz-addresses-pilot sa 
...
CREATE INDEX idx_rna_nza_id_p ON nz-addresses-pilot USING btree (rna_id);
CREATE INDEX idx_rna_nza_id_is_odd_p ON nz-addresses-pilot USING btree (rna_id,is_odd);
