...
copy( select st_x(st_startpoint(the_geom)),st_y(st_startpoint(the_geom)), cn.label,'Shouldbe: '||array_to_string(array_agg(sb.name_ascii),', ')
	...
	join ( select
		...
		on st_intersects(st_transform(the_geom,2193),nztm_geometry)) sb on sb.gid = cn.gid
	where nzslid is not null 
	and not st_intersects(st_transform(the_geom,2193),nztm_geometry)  group by cn.gid order by array_to_string(array_agg(sb.name_ascii),', ')
) to :outfile with CSV;
