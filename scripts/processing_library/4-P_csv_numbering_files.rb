...
def pre_processing()
  ...
  sql_query = "SELECT  DISTINCT ON (road_id, linz_numb_id, address_number) address_number, full_road_name_ascii, to_char(shape_x,'9999D999999'), to_char(shape_y,'9999D999999'), road_id, linz_numb_id FROM nz_addresses_pilot WHERE road_id is not null and ST_Contains(ST_SetSRID(ST_MakeBox2D(ST_Point(#{left}, #{bottom}), ST_Point(#{right} ,#{top})),#{WORKING_SRID}), wkb_geometry);"
