...
def do_options(options)
	if options[:suburbs] then
		options[:SALfrom] = get_from_date(LAST_SAL)
		print "Getting last SAL\n" if DEBUG
	end
...
def get_linz_updates(options)
	...
	if (options[:suburbs])
		...
		puts("Getting #{SALO[:tbln]} updates from #{options[:SALfrom]} to #{to_date}")
		LOGFILE.puts("Getting #{SALO[:tbln]} updates from #{options[:SALfrom]} to #{to_date} at #{options[:currtime]}")
	end
...
#use ogr to import csv files into postgres
	...
	if (options[:suburbs])
		system(proj_lib,"#{ogr_cmd} --config PG_USE_COPY TRUE -overwrite -f \"PostgreSQL\" \"PG:host=localhost user=postgres  dbname=nzopengps\" -lco OVERWRITE=yes  #{SALO[:csfn]}.vrt") or abort("Failed to run #{ogr_cmd} on #{SALO[:csfn]}")
	end
...
#road table
	if (options[:suburbs])
		@conn.exec "DELETE FROM #{SALO[:tbln]} sal USING #{SALO[:csfn]} cs
			WHERE sal.id = cs.id
		...
		"SELECT "\
			" id, name, name_ascii, additional_name, additional_name_ascii, type, major_name, major_name_ascii, major_name_type, "\
			" population_estimate, territorial_authority, territorial_authority_ascii, wkb_geometry, updated "\
		"FROM #{SALO[:csfn]} where __change__ = 'UPDATE'"
	end
